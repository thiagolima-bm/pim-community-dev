...
class TokenController
{
    ...
    protected const CONTENT_MAX_SIZE = 300;
    ...
    {
        $lengt = strlen($request->getContent());
        if (self::CONTENT_MAX_SIZE < strlen($request->getContent())) {
            throw new HttpException(
                Response::HTTP_REQUEST_ENTITY_TOO_LARGE,
                sprintf('Request content exceeded the maximum allowed size of %s bytes', self::CONTENT_MAX_SIZE)
            );
        }
